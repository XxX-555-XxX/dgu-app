...
const defaultEta = () => toISODate(addDays(new Date(), 3));
...
export default function FormsSectionComponent({ assets, onCreate }: FormsSectionProps) {
  ...
  const [eta, setEta] = useState<string>(defaultEta());
  ...
  const selectedAsset = useMemo(
    () => assets.find((a) => a.code === assetCode),
    [assets, assetCode],
  );
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.filter((a) => {
      ...
      const bm = [a.brand, a.model].join(" ").toLowerCase();
      return serial.includes(q) || code.includes(q) || bm.includes(q);
    });
  }, [assets, assetSearch]);
  ...
  function submit() {
    ...
    setType(nextType);
    setEta(defaultEta()); // вместо calcDefaultEta(nextType)
    setPriority("2");
  }
    ...
    <Card className="rounded-2xl">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>Новый наряд ТО / Ремонт</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 gap-3">
                ...
                <Popover open={assetComboOpen} onOpenChange={setAssetComboOpen}>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className="w-full justify-between rounded-xl mt-1"
                    >
                      {selectedAsset ? (
                        <span>
                          {selectedAsset.code} • {selectedAsset.brand} {selectedAsset.model} •{" "}
                  ...
                  <PopoverContent className="p-0 w-[--radix-popover-trigger-width]">
                    <Command>
                      <CommandInput
                        placeholder="Поиск по зав. № / коду / модели"
                        value={assetSearch}
                            ...
                            >
                              <Check
                                className={`mr-2 h-4 w-4 ${
                                  a.code === assetCode ? "opacity-100" : "opacity-0"
                                }`}
                              />
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    const t = v as "PM" | "CM";
                    setType(t);
                    setEta(defaultEta()); // вместо calcDefaultEta(t)
                  }}
                >
            ...
            <div>
              ...
              <Input
                placeholder="Кратко опишите работы"
                value={desc}
